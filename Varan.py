...
logging.basicConfig(level=logging.DEBUG,
                    format=LOG_FORMAT)
logger = logging.getLogger('VariantAnnotation')
...
@VariantAnnotationToolNameSpace.route(
    "/HGVS/<string:select_transcripts>/<string:transcript_model>/<string:genome_build>")
...
@VariantAnnotationToolNameSpace.route("/LOVD/<string:variant_description>/<string:transcript_model>/<string:liftover"
                                      ">/<string:genome_build>/<string:checkonly>/<string:select_transcripts>")
...
class GenomicAnnotaterTool(Resource):
    ...
    def get(self, variant_description, transcript_model, genome_build, liftover, checkonly, select_transcripts):
        args = parser.parse_args()
        genomic_transcript = Main.call_module3_function(variant_description, transcript_model, genome_build, liftover,
                                                        checkonly, select_transcripts)
...
@VariantAnnotationToolNameSpace.route("/extraAnnotation/<string:genomic_transcript>/<string:select_extraannotaion>")
...
class ExtraAnnotations(Resource):
    ...
    def get(self, genomic_transcript, select_extraannotaion):
        ...
        if select_extraannotaion == "vep":
            extra_annotation = Main.call_module4_function(genomic_transcript)
